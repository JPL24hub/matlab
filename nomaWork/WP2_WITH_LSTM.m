...
for k=1:length(EbN0)
     ...
     for m=1:N_iter
      ...
      %h_ab  a=usr b=antena
        ...
        numr1 = T*B*C - T*A*D - D*S*C + D*A*T;%(T-(D*r2));
        denr1 = B*C*C - A*D*C;%C;
        r1 = numr1/denr1;
...
%% collect channel  and r data
       ...
       to = 64;
        channelData{cnt,1} = [a;b;c;d;s;t];
        RData{cnt,1} = [R1;R2;numr2;denr2;numr1;denr1];
        cnt = cnt + 1;
        ...
        inputData2 = [b;c;a;d];
        Pred2 = net2(inputData2);
        r2 = diag(Pred1)/diag(Pred2);
        
        %r1
        inputData3 = [a;b;c;d;s;t];
        Pred3 = net3(inputData3);
        inputData4 = [b;c;a;d];
        Pred4 = net4(inputData4);
        r1 = diag(Pred3)/diag(Pred4);
...
semilogy(EbN0,berr1,'or-','LineWidth',2);  hold on;
semilogy(EbN0,ber1r2,'*b-','LineWidth',2);  hold on;
semilogy(EbN0,berrr2,'g-','LineWidth',2); hold on;
xlabel('SNR (dB)');
ylabel('Bit Error Rate (BER)');
grid on;
legend('user-1','user-2','user-1 listenTo user-2','user-1 neural','user-2 neural','user-1 listenTo user-2 neural');
