...
%  function [ber_theo1,ber_theo2] = noisy_NOMA_BER_Rayleigh()
...
for u = 1:p
    ...
    snr = Pt(u)+10*log10(1*(1/1));
    
%     h_del = (e(1,k)*(10^(-snr/10)))*randn(1,N) + 1i*(e(1,k)*(10^(-snr/10)))*randn(1,N);
    h_del = sqrt(d2^-eta)*(randn(1,N)+1i*randn(1,N))/sqrt(2);
    ...
    ber_theo1(u) = 0.25*(2 - sqrt(gam_a/(2+gam_a)) - sqrt(gam_b/(2+gam_b)));
    ...
    ber_theo2(u) = 0.5*gc - 0.25*gd + 0.25*(ge+gf-gg);
    
    gamma1(u) = a1*pt(u)*mean(g1)/(a2*pt(u)*mean(g1) + no);
    gamma2(u) = a2*pt(u)*mean(g2)/no;
end
%%
[ber_theo11,ber_theo22] = NOMA_BER_Rayleigh();
%%
semilogy(Pt, ber_theo1,'r', 'linewidth',1.5); hold on; grid on;
semilogy(Pt, ber_theo2,'b', 'linewidth',1.5);
semilogy(Pt, ber_theo11, '*r','linewidth',1.5);hold on;
semilogy(Pt, ber_theo22, '*b','linewidth',1.5);
