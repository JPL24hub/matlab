...
G=[zeros(Lch-1,Nfft-Lch+1) eye(Lch-1);eye(Nfft)];
...
Nbps=2; M=2^Nbps;  % Modulation order=2/4/6 for QPSK/16QAM/64QAM
Nfft=64;           % FFT size
Ng=Nfft/4; %Ng=3; %    % Ng=0: Guard interval length
%Ng=0;%Nfft/4;
...
EbN0=[0:5:30];    % EbN0
N_iter=6000;%1e5;       % Number of iterations for each EbN0
...
%   if EbN0(i+1) >=15 
%       N_iter=1000;%1e5;       % Number of iterations for each EbN0
%   else N_iter=100;
%   end
   Neb=0; Ntb=0; % Initialize the number of error/total bits
   for m=1:N_iter
      % Tx______________________________________________________________
      X= randi(1,Nused*Nframe,M); % bit: integer vector %Non binary data 0 1 2 3
