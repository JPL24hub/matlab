...
%
N_iter=6000;%1e5;   % no of packets   % Number of iterations for each EbN0
...
for i=1:length(EbN0)
 ...
 %% Round 2 -Antenna 2
      ...
      %% Precoding matrix 
      %h_ab  a=usr b=antena
      F1= abs(diag(H11r1)).^(2)+abs(diag(H12r2)).^(2);
      F2= abs(diag(H21r1)).^(2)+abs(diag(H22r2)).^(2);
      
      M_1a=F1*abs(diag(H22r2)).^(2)*(abs(diag(H22r2)).^(2)*abs(diag(H11r1)).^(2)-abs(diag(H21r1)).^(2)*abs(diag(H12r2)).^(2))^-1;
      M_1b=-(M_1a*abs(diag(H21r1)).^(2))/abs(diag(H22r2).^(2));

      M_2a=F2*abs(diag(H12r2)).^(2)*(abs(diag(H21r1)).^(2)*abs(diag(H12r2)).^(2)-abs(diag(H22r2)).^(2)*abs(diag(H11r1)).^(2))^-1;
