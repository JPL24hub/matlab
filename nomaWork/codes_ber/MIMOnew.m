...
% function [Eb_N0_dB,theoryBer_nRx1,theoryBerMRC_nRx2,simBer]=MIMOwithZFmfile()
...
for ii = 1:length(Eb_N0_dB)
    ...
    h11 = 1/sqrt(2)*[randn(nRx,nTx,N/nTx) + 1i*randn(nRx,nTx,N/nTx)]; % Rayleigh channel
    h12 = 1/sqrt(2)*[randn(nRx,nTx,N/nTx) + 1i*randn(nRx,nTx,N/nTx)]; % Rayleigh channel
    h21 = 1/sqrt(2)*[randn(nRx,nTx,N/nTx) + 1i*randn(nRx,nTx,N/nTx)]; % Rayleigh channel
    h22 = 1/sqrt(2)*[randn(nRx,nTx,N/nTx) + 1i*randn(nRx,nTx,N/nTx)]; % Rayleigh channel
    
    
    n1 = 1/sqrt(2)*[randn(nRx,N/nTx) + 1i*randn(nRx,N/nTx)]; % white gaussian noise, 0dB variance
    n1 = 1/sqrt(2)*[randn(nRx,N/nTx) + 1i*randn(nRx,N/nTx)]; % white gaussian noise, 0dB variance

    % Channel and noise Noise addition
    y = squeeze(sum(h11.*sMod,2)) + 10^(-Eb_N0_dB(ii)/20)*n1;
    ...
    hCof(1,1,:) = sum(h11(:,2,:).*conj(h11(:,2,:)),1);  % d term
    hCof(2,2,:) = sum(h11(:,1,:).*conj(h11(:,1,:)),1);  % a term
    hCof(2,1,:) = -sum(h11(:,2,:).*conj(h11(:,1,:)),1); % c term
    hCof(1,2,:) = -sum(h11(:,1,:).*conj(h11(:,2,:)),1); % b term
    ...
    hMod =  reshape(conj(h11),nRx,N); % H^H operation
